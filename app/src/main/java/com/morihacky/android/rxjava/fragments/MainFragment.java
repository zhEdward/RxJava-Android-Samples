...
public class MainFragment extends BaseFragment {
    ...
    void demoTimeoutIntervalDelays() {
    ...
    void demoPaging() {
        AlertDialog.Builder build = new AlertDialog.Builder (getActivity ()).
                setTitle ("please choice one mode").
                setSingleChoiceItems (new CharSequence[]{"manual", "auto"}, -1, (dialog, which) -> {
                    if (which == 0) clickedOn (new PaginationFragment ());
                    else clickedOn (new PaginationAutoFragment ());
                    dialog.dismiss ();
    ...
    private void clickedOn(@NonNull Fragment fragment) {
        ...
        getActivity ().getSupportFragmentManager ().beginTransaction ().addToBackStack (tag).replace (android.R.id.content, fragment, tag).commit ();
    }
}
